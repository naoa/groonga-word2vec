...
word2vec_load(grn_ctx *ctx, grn_user_data *user_data)
{
  ...
  for (b = 0; b < words; b++) {
    a = 0;
    while (1) {
      load_vocab[b * max_w + a] = fgetc(f);
      if (feof(f) || (load_vocab[b * max_w + a] == ' ')) break;
      if ((a < max_w) && (load_vocab[b * max_w + a] != '\n')) a++;
    }
    load_vocab[b * max_w + a] = 0;
...
int binary = 1, cbow = 1, debug_mode = 2, window = 5, min_count = 5, num_threads = 12, min_reduce = 1;
int *vocab_hash;
                       ...
                       grn_user_data *user_data)
{
  ...
  if(table_name != NULL && column_names != NULL) {
    if(column_to_train_file(ctx, train_file,
                            ...
                            input_filter, input_add_prefix, input_add_prefix_second, mecab_option) == GRN_TRUE)
     {
